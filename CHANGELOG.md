...
- **Chum & spear fishing** — use (or just throw) a pile of guts at half health or better into water at least ~6ft deep (`MinWaterDepth`, default 1.8m) and a school of 4–5 medium/small fish spawns and swims around the chum point for 3 minutes (`FishCountMin/Max`, `DurationSeconds`). The vanilla spears (Stone and Bone) get a new **Spear Fish** action: stab a school fish and it rides the spear in a new `GebSpearedFish` attachment slot (drops at your feet if the spear already carries one). Works in fresh and salt water with a weighted, environment-filtered `Catches` table under `ChumSettings` in `general.json`; big two-handed trophy fish deliberately can't be speared. The attached fish shows on the spear in the inventory screen only — the vanilla spear model has no attachment proxy to render it in-world
