class PrepareHumpheadwrasse extends PrepareFish
{
	int ran;	
	override void Init()
	{
		...
		m_AnimationLength = 2.1;//animation length in relative time units
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"Humpheadwrasse");//you can insert multiple ingredients this way
		...
		//----------------------------------------------------------------------------------------------------------------------
		ran = Math.RandomFloatInclusive(2.0,4.0);
		for (int i = 0; i < ran; ++i){
			AddResult("HumpheadwrasseFilletMeat");//add results here
