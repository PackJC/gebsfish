class PrepareWalleye extends PrepareFish
{
	int ran;	
	override void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"geb_Walleye");//you can insert multiple ingredients this way
		...
		//----------------------------------------------------------------------------------------------------------------------
		ran = Math.RandomFloatInclusive(m_gebsConfig.Walleye.MeatMin,m_gebsConfig.Walleye.MeatMax);
		for (int i = 0; i < ran; ++i){
			AddResult("geb_WalleyeFilletMeat");//add results here
