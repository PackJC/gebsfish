class PrepareRedheadcichlid extends PrepareFish
{
	int ran;	
	override void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"geb_Redheadcichlid");//you can insert multiple ingredients this way
		...
		//----------------------------------------------------------------------------------------------------------------------
		ran = Math.RandomFloatInclusive(m_gebsConfig.RedheadCichlid.MeatMin,m_gebsConfig.RedheadCichlid.MeatMax);
		for (int i = 0; i < ran; ++i){
			AddResult("geb_RedheadcichlidFilletMeat");//add results here
