class PrepareLeopardshark extends PrepareFish
{
	...
	override void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"geb_Leopardshark");//you can insert multiple ingredients this way
		...
		//----------------------------------------------------------------------------------------------------------------------
		ran = Math.RandomFloatInclusive(m_gebsConfig.LeopardShark.MeatMin,m_gebsConfig.LeopardShark.MeatMax);
		for (int i = 0; i < ran; ++i){
			AddResult("geb_LeopardsharkFilletMeat");//add results here
