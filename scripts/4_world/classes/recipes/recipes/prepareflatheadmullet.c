class PrepareFlatheadmullet extends PrepareFish
{
	int ran;	
	override void Init()
	{
		...
		auto parts = fillets_chance_map.Get("geb_Flatheadmullet");
		...
		m_AnimationLength = 1.3;//animation length in relative time units
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"geb_Flatheadmullet");//you can insert multiple ingredients this way
		...
		//----------------------------------------------------------------------------------------------------------------------
		for (int i = 0; i < ran; ++i){
			AddResult("geb_FlatheadmulletFilletMeat");//add results here
