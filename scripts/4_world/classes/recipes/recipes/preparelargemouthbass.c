class PrepareLargemouthbass extends PrepareFish
{
	int ran;	
	override void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"geb_Largemouthbass");//you can insert multiple ingredients this way
		...
		//----------------------------------------------------------------------------------------------------------------------
		ran = Math.RandomFloatInclusive(m_gebsConfig.LargemouthBass.MeatMin,m_gebsConfig.LargemouthBass.MeatMax);
		for (int i = 0; i < ran; ++i){
			AddResult("geb_LargemouthbassFilletMeat");//add results here
