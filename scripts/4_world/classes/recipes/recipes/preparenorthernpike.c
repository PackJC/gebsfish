class PrepareNorthernpike extends PrepareFish
{
	int ran;
	override void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"geb_Northernpike");//you can insert multiple ingredients this way
		...
		//----------------------------------------------------------------------------------------------------------------------
		ran = Math.RandomFloatInclusive(m_gebsConfig.NorthernPike.MeatMin,m_gebsConfig.NorthernPike.MeatMax);
		for (int i = 0; i < ran; ++i){
			AddResult("geb_NorthernpikeFilletMeat");//add results here
