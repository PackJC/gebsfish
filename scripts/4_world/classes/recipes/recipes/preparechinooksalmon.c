class PrepareChinooksalmon extends PrepareFish
{
	int ran;	
	override void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"geb_Chinooksalmon");//you can insert multiple ingredients this way
		...
		//----------------------------------------------------------------------------------------------------------------------
		ran = Math.RandomFloatInclusive(m_gebsConfig.ChinookSalmon.MeatMin,m_gebsConfig.ChinookSalmon.MeatMax);
		for (int i = 0; i < ran; ++i){
			AddResult("geb_ChinooksalmonFilletMeat");//add results here
