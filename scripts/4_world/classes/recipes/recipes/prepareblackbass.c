class PrepareBlackbass extends PrepareFish
{
	...
	override void Init(){
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"geb_Blackbass");//you can insert multiple ingredients this way
		...
		//----------------------------------------------------------------------------------------------------------------------
		ran = Math.RandomFloatInclusive(m_gebsConfig.BlackBass.MeatMin,m_gebsConfig.BlackBass.MeatMax);
		for (int i = 0; i < ran; ++i){
