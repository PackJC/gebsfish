class PrepareBonita extends PrepareFish
{
	int ran;	
	override void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"geb_Bonita");//you can insert multiple ingredients this way
		...
		//----------------------------------------------------------------------------------------------------------------------
		ran = Math.RandomFloatInclusive(m_gebsConfig.Bonita.MeatMin,m_gebsConfig.Bonita.MeatMax);
		for (int i = 0; i < ran; ++i){
