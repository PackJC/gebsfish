class PrepareSunfish extends PrepareFish
{
	int ran;	
	override void Init()
	{
		...
		//INGREDIENTS
		//ingredient 1
		InsertIngredient(0,"geb_Sunfish");//you can insert multiple ingredients this way
		...
		//----------------------------------------------------------------------------------------------------------------------
		ran = Math.RandomFloatInclusive(m_gebsConfig.Sunfish.MeatMin,m_gebsConfig.Sunfish.MeatMax);
		for (int i = 0; i < ran; ++i){
			AddResult("geb_SunfishFilletMeat");//add results here
