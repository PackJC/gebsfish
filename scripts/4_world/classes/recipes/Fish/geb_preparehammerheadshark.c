class PrepareHammerHeadShark extends PrepareFish {
	...
	override void Init(){
		...
		//INGREDIENTS-----------------------------------------------------------------------------------------------------------
		//Ingredient 1
		InsertIngredient(0,"geb_HammerHeadShark");//you can insert multiple ingredients this way
		...
		//RESULTS----------------------------------------------------------------------------------------------------------------
		//Result 1
		ran = Math.RandomInt(m_gebsConfig.LeopardShark.MeatMin,m_gebsConfig.LeopardShark.MeatMax);
		for (int i = 0; i < ran; ++i){
			AddResult("geb_HammerHeadSharkFilletMeat");//add results here
