 class RepairRodActionReciveData : ActionReciveData
 {
  string m_DamageZoneRecived;
 }
  
 class RepairRodActionData : ActionData
 {
  string m_DamageZone;
 }
  
 class ActionRepairRodCB : ActionContinuousBaseCB
 {
 ...
 class ActionRepairRod: ActionContinuousBase
 {
  typename m_LastValidType; //legacy stuff
  string m_CurrentDamageZone = "";
  int m_LastValidComponentIndex = -1; //legacy stuff
  
  void ActionRepairRod()
  {
  m_CallbackClass = ActionRepairRodCB;
  ...
  if ( !targetParent || !targetParent.IsInherited(FishingRod) )
  return false;
  ...
  FishingRod rod = FishingRod.Cast( targetParent );
  
  string damageZone = "";
  
  for (int s = 0; s < selections.Count(); s++)
  {
  if ( DamageSystem.GetDamageZoneFromComponentName(rod, selections[s], damageZone) ) //NOTE: relevant fire geometry and view geometry selection names MUST match in order to get a valid damage zone
  {
  ...
  if ( damageZone != "" )
  {
  if (module_repairing.CanRepair(item,rod,damageZone))
  {
  ...
  string damageZone = RepairRodActionData.Cast(action_data).m_DamageZone;
  if (!GetGame().IsMultiplayer())
  damageZone = m_CurrentDamageZone;
  
  if ( targetParent && targetParent.IsInherited(FishingRod) && damageZone != "" ) 
  {
  FishingRod rod = FishingRod.Cast( targetParent );
  PluginRepairing module_repairing;
  Class.CastTo(module_repairing, GetPlugin(PluginRepairing));
  
  RepairDamageTransfer(action_data.m_Player,action_data.m_MainItem,rod,m_SpecialtyWeight,damageZone);
  module_repairing.Repair(action_data.m_Player,action_data.m_MainItem,rod,m_SpecialtyWeight,damageZone);
  }
  }
  
  override ActionData CreateActionData()
  {
  RepairRodActionData action_data = new RepairRodActionData;
  return action_data;
  }
  ...
  RepairRodActionData repair_action_data;
  ...
  {
  action_recive_data = new RepairRodActionReciveData;
  }
  super.ReadFromContext(ctx, action_recive_data);
  RepairRodActionReciveData recive_data_repair = RepairRodActionReciveData.Cast(action_recive_data);
  ...
  RepairRodActionReciveData recive_data_repair = RepairRodActionReciveData.Cast(action_recive_data);
  RepairRodActionData.Cast(action_data).m_DamageZone = recive_data_repair.m_DamageZoneRecived;
  }
